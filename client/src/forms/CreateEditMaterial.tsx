...
// @ts-ignore
import {addMaterial, updateMaterial} from "../store/features/materialSlice"
...
const createOnSubmit = (values: any): Promise<any> => {
  return api({
    method: "post",
    url: "/material",
    data: values,
...
export default function CreateEditMaterial(props: CreateEditMaterialProps) {
  ...
  if (
    ...
    // or they dont want to update and we are passing in weird stuff
    || (!props.update && (props.updateObjectId != null || props.initialValues != null))
  ) {
        ...
        if (props.update && props.updateObjectId != null) {
          updateOnSubmit(values, props.updateObjectId).then(async (response) => {
            if (response.data.id) {
              ...
              resetForm()
              enqueueSnackbar("Material Updated", {
                variant: "success",
              })
              props.onCancel(false)
            } else {
              enqueueSnackbar("There was a problem updating the material, code: 104", {
                variant: "error",
              })
            }
          })
            .catch((error) => {
              ...
              enqueueSnackbar("There was a problem updating the material, code: 102", {
                variant: "error",
              })
            })
            .finally(() => {
          ...
          createOnSubmit(values).then(async (response) => {
            if (response.data.id) {
              const newMaterial = response.data
              dispatch(addMaterial(newMaterial))

              resetForm()
              enqueueSnackbar("Material Created", {
                variant: "success",
              })
              props.onCancel(false)
            } else {
              enqueueSnackbar("There was a problem creating the material, code: 104", {
                variant: "error",
              })
            }
          })
            .catch((error) => {
              console.log("error", error)
              enqueueSnackbar("There was a problem creating the material, code: 102", {
                variant: "error",
              })
            })
            .finally(() => {
